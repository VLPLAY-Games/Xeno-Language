...
- По бенчмаркам Xeno примерно **~22× медленнее, чем эквивалентный нативный C++**, в зависимости от нагрузки (см. `benchmark.ino`).  
...
|-----------------------------|:-----------------:|:------------------------:|:----------------------:|:-------------------------:|
| Скорость выполнения (vs C++)| ~22× медленнее    | ~18× медленнее *(прибл.)*| ~20× медленнее *(прибл.)*| 🥇 Базовый уровень         |
...
### Установка
Скопируйте папку `xenoLang` в ваш Arduino-проект. В репозитории также есть примеры `.ino` в папке `examples/`.
